...
<body>
<h1 align="center"> Edit Testimoni</h1>
<?php
//ambil data berdasarkan parameter GET id
$b = mysqli_fetch_array(mysqli_query($db, "SELECT * FROM testimoni where idtestimoni='$_GET[idtestimoni]'"));
...
if(isset($_POST['simpan'])){
    if(empty($file)){
            $save=mysqli_query($db, "UPDATE testimoni set gambar='$lama' where idtestimoni='$_GET[idtestimoni]'");
            if($save){ //jika update berhasil maka muncul pesan dan menuju ke halaman produk
                       echo "<script>alert('Data Produk Berhasil disimpan ke database');document.location='testimonisetting.php'</script>";
            }else{  //jika update gagal maka muncul pesan
                     echo "<script>alert('Proses simpan gagal, coba kembali');document.location='testimonisetting.php'</script>";
             }
    }
    else{
    ...
    $save=mysqli_query($db, "UPDATE testimoni set gambar='$nama_file' where idtestimoni='$_GET[idtestimoni]'");
    if($save){ //jika update berhasil maka muncul pesan dan menuju ke halaman produk
        echo "<script>alert('Gambar Testimoni Berhasil disimpan ke database');document.location='testimonisetting.php'</script>";
    }else{  //jika update gagal maka muncul pesan
         echo "<script>alert('Proses simpan gagal, coba kembali');document.location='testimonisetting.php'</script>";
    }
}
